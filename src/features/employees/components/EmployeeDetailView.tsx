...
/**
 * Mock worm earnings data for chart
 */
const generateMockWormData = (employee: Employee) => {
  const data = [];
 ...
 */
const generateMockTransactions = (employee: Employee) => {
  const transactions = [];
  const currentDate = new Date();
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const types = ['earn', 'spend', 'transfer', 'adjustment'];
    const wormTypes = ['gold', 'silver', 'bronze'];
    ...
    transactions.push({
      ...
      worm_type: wormTypes[Math.floor(Math.random() * wormTypes.length)],
      amount: Math.floor(Math.random() * 10) + 1,
      value_usd: (Math.floor(Math.random() * 10) + 1) * (wormTypes[Math.floor(Math.random() * wormTypes.length)] === 'gold' ? 10 : wormTypes[Math.floor(Math.random() * wormTypes.length)] === 'silver' ? 3 : 1),
...
function getPermissionInfo(permission: EmployeePermission) {
  switch (permission) {
    case 'admin':
      return { icon: '👑', label: 'Admin', description: 'Full administrative access' };
    case 'approve_distributions':
      return { icon: '✅', label: 'Approve Distributions', description: 'Can approve all worm distributions' };
    case 'approve_small_distributions':
      return { icon: '✔️', label: 'Approve Small Distributions', description: 'Can approve small worm distributions' };
    case 'view_all_balances':
      return { icon: '👁️', label: 'View All Balances', description: 'Can view all employee balances' };
    case 'view_team_balances':
      return { icon: '👥', label: 'View Team Balances', description: 'Can view team member balances' };
    case 'view_own_balance':
      return { icon: '👤', label: 'View Own Balance', description: 'Can view own worm balance' };
    case 'manage_employees':
      return { icon: '👥', label: 'Manage Employees', description: 'Can manage employee accounts' };
    case 'create_policies':
      return { icon: '📋', label: 'Create Policies', description: 'Can create and modify policies' };
    default:
      return { icon: '❓', label: permission, description: 'Unknown permission' };
  }
}
...
function getStatusInfo(status: string) {
  switch (status) {
    case 'active':
      return { icon: '✅', label: 'Active', variant: 'default' as const };
    case 'inactive':
      return { icon: '⏸️', label: 'Inactive', variant: 'secondary' as const };
    case 'terminated':
      return { icon: '❌', label: 'Terminated', variant: 'destructive' as const };
    case 'on_leave':
      return { icon: '🏖️', label: 'On Leave', variant: 'outline' as const };
    default:
      return { icon: '❓', label: status, variant: 'outline' as const };
  }
}
